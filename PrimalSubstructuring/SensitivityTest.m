...
% defect amplitudes
xi1 = 0;    % defect amplitude - roughness on top blade (x)
xi2 = 0;	   % defect amplitude - roughness on top blade(z)
xi3 = 0.2;     % defect amplitude - top blade angle (x)
xi4 = 0.3;     % defect amplitude - top blade angle (z)
xi5 = 0.3;     % defect amplitude - bottom blade angle (y)
xi6 = 0.4;     % defect amplitude - left blade angle (z)
...
Omega = [0 0 0]; % rotation speed in rad/s
...
roughness_defect_x = zeros(numel(nodes),1);
roughness_defect_x(1:3:end) = vd1; % vectorized defect-field
...
roughness_defect_z = zeros(numel(nodes),1);
roughness_defect_z(3:3:end) = vd1; % vectorized defect-field
...
xi = [xi1 xi2 xi3 xi4 xi5 xi6]'; % parameter vector
...
NominalPrimalSub.DATA.D = alfa*NominalPrimalSub.DATA.M + beta*NominalPrimalSub.DATA.K;
NominalPrimalSub.DATA.Dc = NominalPrimalSub.constrain_matrix(NominalPrimalSub.DATA.D);
...
% Damping _________________________________________________________________
DefectedPrimalSub.DATA.D = alfa*DefectedPrimalSub.DATA.M + beta*DefectedPrimalSub.DATA.K;
DefectedPrimalSub.DATA.Dc = DefectedPrimalSub.constrain_matrix(DefectedPrimalSub.DATA.D);
...
DOFs = NominalPrimalSub.Us{2}(DOFs);
...
omegas = 2*pi*freqs;
for i=1:fn
    G = -omegas(i)^2*NominalPrimalSub.DATA.Mc +1i*omegas(i)*NominalPrimalSub.DATA.Dc + NominalPrimalSub.DATA.Kc;
    NominalFRF(:,i) = NominalPrimalSub.unconstrain_vector(G\Fextc_sub);
    G = -omegas(i)^2*DefectedPrimalSub.DATA.Mc +1i*omegas(i)*DefectedPrimalSub.DATA.Dc + DefectedPrimalSub.DATA.Kc;
...
% On Nominal
[Mn_cb,Kn_cb,Tn_cb,Ln_cb] = CraigBamptonReduction(NominalPrimalSub,200);
...
% On defected
[Md_cb,Kd_cb,Td_cb,Ld_cb] = CraigBamptonReduction(DefectedPrimalSub,200);

[VMd_cb,om] = eigs(Kd_cb, Md_cb, n_VMs, 'SM');
[f0d_cb,ind] = sort(sqrt(diag(om))/2/pi);

% disp eigenfrequencies in command window
disp(table(f0n, f0_ROMn, f0n_cb, f0d, f0_ROMd, f0_DpROM, f0_DpROM_Mxi,f0d_cb))